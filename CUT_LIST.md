...
| Cut | Why | Cost to add |
|---|---|---|
| **`require_ingredient` — a positive ingredient preference** | The constraint vocabulary is asymmetric by design, and the asymmetry is visible: five kinds express avoidance (`exclude_tag`, `exclude_ingredient`, `exclude_cuisine`, `budget_max`, `max_prep_minutes`), and only three express attraction — `require_tag`/`require_any_tag`, which take a tag, and `cuisine_affinity`, which takes a cuisine. Tags are derived from allergen, religious and dietary data, so they are a *safety* vocabulary, not a taste one: there is no `chicken` tag and no way to say "chicken please". A diner asking for one gets it through retrieval and ranking, never through a filter — which is the correct default, because a taste should reorder your options, not eliminate them. Observed live: free text meaning "chicken lover" compiled to **no constraint at all** (correctly — the model declined to force it into a wrong-shaped kind) and the request was still honoured by the reranker. | An `@evaluator("require_ingredient", shape=..., parse=...)` registration, canonicalising through `trusted.canonical.resolve_ingredient_id` exactly as `exclude_ingredient` already does, and returning `satisfied`/`violated` on membership in `recipe.ingredient_ids`. The registry makes this genuinely additive — one registration, no branch to edit. **The severity is the whole design question, not the code:** it must never be floored into `HARD_SEVERITIES`, or "I like chicken" starts producing `NEGOTIATE` on a catalog with no legal chicken dish. It belongs at `GOAL`, where two-pass selection prefers it and feasibility never blocks on it. Roughly 2 hours including the evaluator, the parser, a golden case, and an eval confirming it changes ranking without changing the legal set. |
